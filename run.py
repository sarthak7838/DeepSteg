...
"""
@author: Sarthak Goel
@contact: sarthakg.co@nsit.net.in
...
CONTAINER_IMAGE_PATH2='example_pics/container_images2/'
...
parser.add_argument('--mode', type=int, default=32,
                    help='input mode 1-Hiding 0-Reveal')
...
def main():
    ...
    if opt.Hnet != "":
        Hnet.load_state_dict(torch.load(opt.Hnet,map_location=lambda storage, loc: storage))
...
def validation(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(val_loader, 0):
        Hnet.zero_grad()
        Rnet.zero_grad()
        #all_pics = data  # allpicscoverImg secretImg,label
        secret_img, cover_img = data
        this_batch_size = int(secret_img.size()[0])
        ...
        concat_imgv = Variable(concat_img, volatile=True)  # concatImg 
        cover_imgv = Variable(cover_img, volatile=True)  # coverImg 
...
def test(test_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        # print(container_img)
        # TENSOR_LIST.append(container_img)
        rev_secret_img = Rnet(container_img)  # containerImg is the input of the Rnet and get the output "rev_secret_img"
...
def save_result_pic(this_batch_size, originalLabelv, ContainerImg, secretLabelv, RevSecImg, epoch, i, save_path):
    # returnimg=cv2.cvtColor(ContainerImg, cv2.COLOR_BGR2RGB)
    # cv2.imwrite(CONTAINER_IMAGE_PATH+'c.jpg',returnimg)
    if not opt.debug:
        ...
        imagepath=CONTAINER_IMAGE_PATH+'c.jpg'
        vutils.save_image(containerFrames,imagepath,nrow=this_batch_size, padding=1, normalize=True)
        ...
        OriginalImgName='%s/ResultPicsOriginal_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        ContainerImgName='%s/ResultPicsContainer_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        ConImgName='%s/frame%03d.png'%(CONTAINER_IMAGE_PATH,i)
        ConImgName2='%s/frame%03d.png'%(CONTAINER_IMAGE_PATH2,i)
        SecretImgName='%s/ResultPicsSecret_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        RevSecretImgName='%s/ResultPicsRevSecret_epoch%03d_batch%04d.png' % (save_path, epoch, i)
        RSecImgName='%s/frame%03d.png'%(FINAL_SECRET_IMAGE_PATH,i)
        # vutils.save_image(originalFrames, OriginalImgName, nrow=this_batch_size, padding=1, normalize=True)
        # vutils.save_image(containerFrames, ContainerImgName, nrow=this_batch_size, padding=1, normalize=True)
        # vutils.save_image(secretFrames, SecretImgName, nrow=this_batch_size, padding=1, normalize=True)
        # vutils.save_image(revSecFrames, RevSecretImgName, nrow=this_batch_size, padding=1, normalize=True)
        # vutils.save_image(resultImg, resultImgName, nrow=this_batch_size, padding=1, normalize=True)
        vutils.save_image(containerFrames, ConImgName, nrow=this_batch_size, padding=1, normalize=True)
        vutils.save_image(containerFrames2, ConImgName2, nrow=this_batch_size, padding=0, normalize=False)
        vutils.save_image(revSecFrames, RSecImgName, nrow=this_batch_size, padding=1, normalize=True)
